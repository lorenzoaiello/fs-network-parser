...
class Vatsim
{
    ...
    private $status_url        = 'https://status.vatsim.net/';
    ...
    private $sections = array(
        'GENERAL' => '_getGeneral',
        'VOICE SERVERS' => '_getVoiceServers',
        'CLIENTS' => '_getClients',
        'PREFILE' => '_getPrefiled',
        'SERVERS' => '_getServers',
    );
    ...
    public function getStatus()
    {
        ...
        if ($code !== 200) {
            throw new \Exception('Error retrieving VATSIM status page. Endpoint returned: ' . $code, $code);
        }
    ...
    public function getData()
    {
        if ($this->data_url == null) {
            $this->data_url = array_rand($this->status_data_clean['url0']);
        }
        
        $response = $this->client->request('GET', $this->status_url);
        $code = $response->getStatusCode();
        if ($code !== 200) {
            throw new \Exception('Error retrieving VATSIM data page. Endpoint returned: ' . $code, $code);
        }
    ...
    {
        if (substr($line, 0, 1) == '!') {
            if (array_key_exists($this->getStringBetween($line, '!', ':'), $this->sections)) {
                $this->currentProcessor = $this->sections[$this->getStringBetween($line, '!', ':')];
                return;
            } else {
                return;
            }
    ...
    private function _getVoiceServers($line)
    {
        $data = explode(':', $line);
        
        if (count($data) >= 5) {
            $this->data_data_clean['voice_servers'][$data[0]] = array(
                'hostname' => $data[0],
                'location' => $data[1],
                'name' => $data[2],
                'new_connections_allowed' => $data[3],
                'type' => $data[4],
            );
        }
    }
    ...
    private function _getPrefiled($line)
    {
        $data = explode(':', $line);
        if (count($data) == 42) {
            $client = $this->_parseClient($data);
            $this->data_data_clean['prefile'][] = $client;
        }
    }
